...
if ($gambar != '') {
    if (in_array($ext_Gambar, $ext)) {
        if ($size < 5000000) {
            move_uploaded_file($folder, "../admin/img/taman/" . $rename);
            $query = mysqli_query($koneksi, "UPDATE member SET sertifikat_taman = '$sertifikat', foto_taman='$rename' WHERE id_member='$id' ");
            if ($query) {
